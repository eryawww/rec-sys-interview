"""(age,gender,region,title-unigrams,content_type,genre) -> watch_seconds > K"""
import numpy as np, pandas as pd
from sklearn.ensemble import HistGradientBoostingClassifier
from sklearn.model_selection import KFold, GroupKFold
from sklearn.metrics import roc_auc_score, accuracy_score
from sklearn.feature_extraction.text import CountVectorizer

ev=pd.read_csv('data/events.csv',parse_dates=['timestamp'])
it=pd.read_csv('data/items.csv'); us=pd.read_csv('data/users.csv')
df=ev.merge(it,on='item_id').merge(us,on='user_id')

cv=CountVectorizer(lowercase=True)
X=np.hstack([
    pd.get_dummies(df[['age','gender','region','content_type','genre']].astype(str)).values.astype(float),
    cv.fit_transform(df.title).toarray()])
print(f'n={len(df)}  features={X.shape[1]} (incl. {len(cv.vocabulary_)} title unigrams)\n')

def run(y, groups=None, splitter=None):
    aucs=[]; accs=[]
    it_ = splitter.split(X,y,groups) if groups is not None else splitter.split(X)
    for tr,te in it_:
        if len(np.unique(y[tr]))<2 or len(np.unique(y[te]))<2: continue
        m=HistGradientBoostingClassifier(random_state=0).fit(X[tr],y[tr])
        p=m.predict_proba(X[te])[:,1]
        aucs.append(roc_auc_score(y[te],p)); accs.append(accuracy_score(y[te],p>0.5))
    return np.mean(aucs), np.std(aucs), np.mean(accs)

rows=[]
for K in [30,60,120,300,600,900,1200,1800,2400,3000]:
    y=(df.watch_seconds>K).values.astype(int)
    pos=y.mean(); maj=max(pos,1-pos)
    a,s,acc = run(y, splitter=KFold(5,shuffle=True,random_state=0))
    ag,sg,accg = run(y, groups=df.item_id.values, splitter=GroupKFold(5))
    # shuffled-label control
    ysh=np.random.default_rng(0).permutation(y)
    ash,_,_ = run(ysh, splitter=KFold(5,shuffle=True,random_state=0))
    rows.append(dict(K=K, pos_rate=pos, majority=maj,
                     AUC_random=a, sd=s, acc_random=acc,
                     AUC_grouped=ag, AUC_shuffled=ash))
d=pd.DataFrame(rows)
pd.set_option('display.width',220)
print(d.to_string(index=False,float_format=lambda x:f'{x:.3f}'))
print('\nAUC 0.500 = chance.  acc must beat `majority` to mean anything.')
